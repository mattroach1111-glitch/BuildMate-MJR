...
# Recent Changes
- **August 15, 2025**: **RESOLVED**: Fixed header button scrolling issue in job sheets. Removed `sticky top-0` positioning from action buttons (Edit, Download PDF, Email PDF, Delete) so they now scroll naturally with content instead of staying fixed at the top. Enhanced mobile UX by ensuring all interactive elements move with the scrollable content area. Working on conditional landscape toggle implementation for job sheets and timesheets only.
