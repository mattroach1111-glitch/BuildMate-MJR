...
## Recent Changes
- **August 12, 2025**: PROJECT MANAGER FILTERED EMAILS - Enhanced email system to filter job updates by project manager. Mark sees only jobs with "Mark" in project name, Will sees only jobs with "Will" in project name. Email subjects automatically include PM name prefix (e.g., "Mark's Job Updates" or "Will's Job Updates"). Admins continue to see all jobs. This ensures project managers only send updates for their assigned projects.
