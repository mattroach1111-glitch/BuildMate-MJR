...
# Recent Changes
- **August 15, 2025**: **RESOLVED**: Fixed employee auto-addition behavior and added selective landscape mode. Updated PDF processing to only add new employees to the specific job they're extracted from (not all jobs), while manually added employees via staff menu still get added to all existing jobs as expected. Added landscape mode toggle for job sheets and timesheets only. Implemented employee management dialog in job sheets allowing admins to view and delete auto-created employees.
- **August 15, 2025**: **RESOLVED**: Fixed sticky header button positioning in job sheets. Completely restructured action button layout - moved Edit, Download PDF, Email PDF, and Delete buttons from top of modal to bottom of scrollable content area. Buttons now appear after all job content sections and scroll naturally with the page instead of staying fixed at the top. Enhanced mobile UX by ensuring proper content flow and eliminating overlay issues.
- **August 15, 2025**: **RESOLVED**: Fixed landscape mode and PDF upload issues. Disabled problematic landscape mode feature due to browser compatibility issues and focused on core functionality. Successfully moved job sheet PDF buttons (Download PDF, Email PDF) from fixed header to scrollable content area for better mobile accessibility. Resolved 401 authentication timeout issue affecting PDF job creation uploads. Users can now physically rotate their phones for landscape viewing, and all core features work reliably.
- **August 15, 2025**: **RESOLVED**: Fixed landscape mode scrolling and menu positioning issues. Updated CSS to enable scrolling in landscape mode by changing body overflow from hidden to auto, and fixed dropdown/popover positioning by applying counter-rotation to portal elements. Landscape mode now works correctly on deployed mobile devices with proper scrolling and menu functionality.
- **August 14, 2025**: **RESOLVED**: Successfully implemented mobile landscape orientation toggle. JavaScript-based rotation with proper viewport centering works correctly on actual mobile devices. The landscape button provides seamless orientation switching for mobile users, with all menus and interactions working identically in both orientations. Feature is optimized for mobile devices and may appear differently when tested on desktop browsers.
- **August 14, 2025**: Enhanced Google Drive integration for universal public file access. All job file attachments and PDF job sheets are now automatically uploaded to Google Drive in organized public folders ("BuildFlow Pro/Job - [Address]/Attachments"). When job sheet PDFs are emailed, they include both email attachments and clickable Google Drive links for easy online access. This ensures all recipients can access documents regardless of email client limitations.
- **August 14, 2025**: **RESOLVED**: Fixed critical Google Drive PDF links issue in email attachments. Root cause: Google Drive upload was creating duplicate file records instead of updating existing ones. Solution: Modified `/api/documents/upload-to-drive` endpoint to find and update existing file records with Google Drive links, ensuring PDF emails contain clickable Google Drive links instead of "(Available in system)" text. User confirmed fix works perfectly in production.
- **August 13, 2025**: Enhanced project manager selection in job creation with professional dropdown interface. Added "Add new project manager" option with inline input mode, Save/Cancel buttons, and immediate availability for future jobs.
