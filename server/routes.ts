...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/debug/timesheets/:employeeId", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.status(500).json({ message: "Debug query failed", error: (error as Error).message });
    }
  });
  ...
  // =============================================================================
  // EMAIL DOCUMENT PROCESSING ENDPOINTS
  // =============================================================================

  // Get pending email documents awaiting job selection
  app.get("/api/email-documents/pending", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      ...
      const pendingDocs = await storage.getPendingEmailDocuments();
      res.json(pendingDocs);
    } catch (error) {
      console.error("Error fetching pending email documents:", error);
      res.status(500).json({ message: "Failed to fetch pending documents" });
    }
  });

  // Assign job to email document and process it
  app.post("/api/email-documents/:documentId/assign-job", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      ...
      // Get the document
      const document = await storage.getEmailProcessedDocument(documentId);
      if (!document) {
        return res.status(404).json({ message: "Document not found" });
      }

      // Update document with job assignment
      await storage.updateEmailProcessedDocument(documentId, {
        jobId: jobId,
        status: 'processed'
      });

      // Add expense to the job sheet based on category
      const extractedData = JSON.parse(document.extractedData);
      const expenseData = {
        jobId: jobId,
        vendor: document.vendor,
      ...
      // Add to appropriate job sheet category
      switch (document.category) {
        case 'materials':
          await storage.createMaterial({
            ...expenseData,
            quantity: document.amount,
            rate: null,
            unit: 'unit'
          });
          break;
        ...
        case 'tip_fees':
          await storage.createTipFee({
            ...expenseData,
            tipSize: 'medium', // Default size
            cartage: document.amount * 0.2 // 20% cartage
          });
          break;
        default: // other_costs
          await storage.createOtherCost(expenseData);
      }

      res.json({ success: true, message: "Document assigned to job and processed" });
    } catch (error) {
      console.error("Error assigning job to document:", error);
      res.status(500).json({ message: "Failed to assign job to document" });
    }
  });
