...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/organiser/weeks", isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.error("Error fetching organiser weeks:", error);
  ...
  // Staff-specific organiser endpoint - only returns current user's schedule
  app.get('/api/organiser/my-schedule', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Only staff users should access this endpoint
      if (user.role === "admin") {
      ...
      // Create staff name from user's name
      const staffName = `${user.firstName || ''} ${user.lastName || ''}`.trim();
      
      if (!staffName) {
        return res.status(400).json({ message: "User name not found. Please ensure your profile has a name set." });
      }

      console.log(`🔒 Staff user ${staffName} requesting organiser data for week ${weekStartDate}`);

      // Get only this staff member's organiser data
      const staffOrganiserData = await storage.getStaffOrganiserData(staffName, weekStartDate as string);
      
      if (!staffOrganiserData) {
        console.log(`🔍 No organiser data found for staff member: ${staffName}`);
        // Return empty schedule structure if no organiser record exists
        const emptySchedule = {
          id: undefined,
          staffId: '',
          staffName: staffName,
      ...
      console.log(`✅ Found organiser data for ${staffName}:`, staffOrganiserData);
      
      // Return as an array for compatibility with frontend (but only one item)
      res.json([staffOrganiserData]);
    } catch (error) {
      console.error("Error fetching staff organiser data:", error);
      res.status(500).json({ message: "Failed to fetch your schedule data" });
    }
  });
